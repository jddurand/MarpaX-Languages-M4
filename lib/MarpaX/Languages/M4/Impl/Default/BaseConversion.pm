...
class MarpaX::Languages::M4::Impl::Default::BaseConversion {
    ...
    method bitvector_fr_base (ClassName $class: PositiveInt $bits, PositiveInt $base, Str $input) {
      #
      ...
      if ($base == 1) {
        $input =~ s/^0*//;
        if ($input =~ /[^1]/) {
          croak "radix 1 imposes eventual leading zeroes followed by zero or more '1' character(s)";
        }
        ...
        if ($num >= $base) {
          ...
          if ($base <= 10) {
            ...
            if ($base == 11) {
              ...
              $range .= 'a-' . $nums[$base - 1] . '] (case independant)';
            }
          }
    ...
    method bitvector_to_base (ClassName $class: PositiveInt $base, ConsumerOf['Bit::Vector'] $v, Int $min --> Str) {
        ...
        if ($base == 1) {
          my $rep = $abs->to_Bin();
          #
          # Remove leading zeroes produced by Bit::Vector
          #
          $rep =~ s/^0*//;
          #
          # Add those because of our width
          #
          if (length($rep) < $min) {
